...
There are two ways to invoke the tools:

1. Run directly from the repo without installing:
   ```bash
   ...
   ```
2. Install the package (editable or pinned) and reuse it anywhere:
   ```bash
   uv pip install -e ./ai-tools
   # or build/install a versioned wheel
   ```
   After installing once you can omit the `--from ./ai-tools` prefix:
   ```bash
   uv tool run fix-issue-with-kilocode 123
   ```

Re-run the editable install command whenever you change files under `ai-tools/`
...
directly via `./ai-tools/...` if you prefer.
